...
function renderManagedHead(metadata, { indexable = true } = {}) {
  const canonical = metadata.path ? absoluteUrl(metadata.path) : null;
  const image = absoluteUrl(metadata.image);
  const structuredData = indexable ? createStructuredData(metadata) : null;
...
for (const route of INDEXED_ROUTES) {
  ...
  await writeFile(outputPath, replaceHead(baseHtml, metadata), 'utf8');
}
