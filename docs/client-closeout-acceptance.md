...
- Casino NITE: preserve the praised design; use the Christmas poker-table photo from Brenton's July `photos` email and replace irrelevant poker/table imagery where appropriate.
...
- Do not deploy, push, send client email, change production configuration/secrets, or mutate external systems.
